...
AD7495::AD7495(SPIClass &spi, uint8_t misoPin, uint8_t csPin)
  : _spi(spi), _misoPin(misoPin), _csPin(csPin), _initialized(false), _spiFreq(1000000) {
  // Create mutex for SPI access
  _spiMutex = xSemaphoreCreateMutex();
}

void AD7495::begin(uint32_t spiFreq) {
  // Configure CS pin
  ...
  // Mark as initialized if mutex was created successfully
  _initialized = (_spiMutex != NULL);
}
